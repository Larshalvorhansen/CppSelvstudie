...
TetrisWindow::TetrisWindow() : 
    ...
    scoreBox{blockSize * tetrisGridWidth + outBoxOffset, 0*sideBarLength, sideBarLength-outBoxOffset, sideBarHeight, "Score"},
    highScoreBox{blockSize * tetrisGridWidth + outBoxOffset, 1*sideBarHeight, sideBarLength-outBoxOffset, sideBarHeight, "High Score"},
    quitBtn{blockSize * tetrisGridWidth ,  2*sideBarHeight, sideBarLength, sideBarHeight,"Quit"},
    restartBtn{blockSize * tetrisGridWidth ,  3*sideBarHeight, sideBarLength, sideBarHeight,"Restart"},
    pauseBtn{blockSize * tetrisGridWidth ,  4*sideBarHeight, sideBarLength, sideBarHeight,"Pause"},

    sideBox{{blockSize * tetrisGridWidth ,  5*sideBarHeight}, sideBarLength, blockSize * tetrisGridHeight-5*sideBarHeight},

    gameEndBackGround{{blockSize*3, 50}, 200, 50}

{
    ...
    highScoreBox.value(std::to_string(highScore).c_str());
    ...
    pauseBtn.callback(cb_pause, this);
}
...
void TetrisWindow::handleInput() {
    ...
    if(currentXKeyState) {
        ...
        scoreBox.value(std::to_string(score).c_str());
    }
...
void TetrisWindow::run() {
    ...
    while(!should_close()) {
        ...
        draw_rectangle({0, 0}, blockSize * tetrisGridWidth, blockSize * tetrisGridHeight, 40);
        drawGridMatrix();
        drawCurrentTetromino();

        if(paused) {
            draw_rectangle({23, 33}, 170, 30);
            draw_text({40, 55}, "Game Paused");
        }

        if(!playing) {
            draw_rectangle({23, 33}, 200, 30, Color::yellow);
            draw_text({40, 55}, "Game has ended");
        }
...
const map<TetrominoType, int> tetrominoColorMap {
    {TetrominoType::J, 4},
    {TetrominoType::L, 91},
    {TetrominoType::T, 152},
    {TetrominoType::S, 2},
    ...
    for(int row = 0; row < tetrisGridHeight; row++) {
        for(int column = 0; column < tetrisGridWidth; column++) {
            ...
            if(field != TetrominoType::NONE) {
                draw_rectangle(Point{column * blockSize, row * blockSize}, blockSize, blockSize, tetrominoColorMap.at(field));
            }
        }
    }
}
...
void TetrisWindow::drawCurrentTetromino() {
    ...
    for(int row = 0; row < size; row++) {
        for(int column = 0; column < size; column++) {
            ...
            if(field != TetrominoType::NONE) {
                draw_rectangle(Point{(topLeftCorner.x + column) * blockSize, (topLeftCorner.y + row) * blockSize}, blockSize, blockSize, tetrominoColorMap.at(field));
            }
        }
    }
}
