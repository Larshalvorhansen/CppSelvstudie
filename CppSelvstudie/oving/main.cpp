...
void pytagoras(){
    ...
    win.draw_quad(point1, point2, Point{point2.x+100,100}, Point{point1.x+100,point1.y-200}, Color::blue);

    win.wait_for_close();
}

void calculateBalance(double innskudd, double rente, int aar){
    cout << "År" << '\t' << "Saldo" << endl;
    vector<double> t(aar); // 10 int av verdien 0
    for(int i = 0; i < aar; i++){
        t[i] = innskudd * (pow((1.0+rente/100.0),i));
        cout << i << '\t' << t[i] << '\n';
    }
}

// void calculateBalanceWithoutVectors(int innskudd, int rente, int aar){
//     for(int i = 0; i <= aar; i++){
...
int main(){
    ...
    //isOdd(7654321);
    printHumanReadableTime(366672);
    //solveQuadraticEquation();
    //pytagoras(); //Fungerte i starten men får nå verken utskrift eller feilmelding, men heller en redirect til @_cxa_throw linje tre
